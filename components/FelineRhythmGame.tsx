...
const FelineRhythmGame: React.FC<FelineRhythmGameProps> = ({ mode, onGameEnd }) => {
    ...
    return (
        <div className="w-full max-w-md mx-auto p-4 bg-gray-800 rounded-lg shadow-lg border-4 border-purple-500 text-white select-none">
            <div className="flex justify-between items-center mb-4 font-bold">
                <div className="text-2xl">Score: {score}</div>
                <div className="text-2xl">Time: {timeLeft}s</div>
            </div>
            
            <div ref={gameAreaRef} className="relative w-full h-[600px] bg-gray-900 overflow-hidden border-2 border-gray-700 rounded-md">
                {displayNotes.map(note => (
                    <div
                        key={note.id}
                        data-id={note.id}
                        className="note absolute"
                        style={{ left: `${(note.lane * 25) + 12.5}%`, transform: 'translateX(-50%)' }}
                    >
                       <MusicNoteIcon className="w-10 h-10 text-pink-400" />
                    </div>
                ))}
                <div className="absolute bottom-0 left-0 right-0 h-[10px] bg-gradient-to-t from-purple-500 to-transparent" style={{bottom: `${TARGET_LINE_FROM_BOTTOM - 5}px`}}/>
            </div>
            ...
            <div className="mt-4 grid grid-cols-4 gap-2">
                {Array.from({ length: LANES }).map((_, i) => (
                    <button 
                        ...
                        className={`py-4 rounded-lg font-bold text-xl transition-colors duration-100 ${
                            hits[i] === 'hit' ? 'bg-green-500' : hits[i] === 'miss' ? 'bg-red-500' : 'bg-purple-600 hover:bg-purple-700'
                        }`}
                    >
                       {['D', 'F', 'J', 'K'][i]}
                    </button>
